...
class CSDI_Forecasting(CSDI_base):
    ...
    def get_side_info(self, observed_tp, cond_mask, feature_id=None):
        B, K, L = cond_mask.shape  # Use actual dimensions from cond_mask

        time_embed = self.time_embedding(observed_tp, self.emb_time_dim)  # (B,L,emb)
        time_embed = time_embed.unsqueeze(2).expand(-1, -1, K, -1)  # Use K from cond_mask

        # FIXED: Use K (actual feature dimension) instead of self.target_dim
        if feature_id is None:
            feature_embed = self.embed_layer(
                torch.arange(K).to(self.device)  # Use K instead of self.target_dim
            )  # (K,emb)
            feature_embed = feature_embed.unsqueeze(0).unsqueeze(0).expand(B, L, -1, -1)
        else:
    ...
    def evaluate(self, batch, n_samples):
        (
            ...
            gt_mask,
            _,
            cut_length,  # FIXED: Now properly captured from process_data
            feature_id, 
        ) = self.process_data(batch)
    ...
    # FIXED: Override impute method specifically for forecasting with proper conditioning
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        """
        FIXED IMPUTE METHOD FOR FORECASTING
        Date: 2025-07-19 07:58:11 UTC | User: ATX1221
        
        Key Fix: Apply conditioning at EVERY step during reverse diffusion to ensure
        observed data is preserved exactly during sequential forecasting.
        """
        B, K, L = observed_data.shape
        imputed_samples = torch.zeros(B, n_samples, K, L).to(self.device)
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                ...
                # CRITICAL FIX: Apply conditioning at EVERY step during reverse diffusion
                # This ensures observed data is preserved exactly during sequential forecasting
